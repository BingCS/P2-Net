...
class P2NETMatchDataset(PointCloudDataset):
    ...
    def __getitem__(self, index):
        ...
        for anc_id in batched_corrs_id:
            ...
            if self.mode == 'train':
                points_noise = np.random.rand(anc_points.shape[0], 3) * self.opt.aug_noise
                anc_points += points_noise
                # anc_points = rotate(anc_points, num_axis=self.opt.aug_rotation)

            anc_keypts = self.keypts[anc_id][:, 0]
            pos_keypts = self.keypts[anc_id][:, 1:]

            selected_ind = np.random.choice(min(len(anc_keypts), len(pos_keypts)), self.opt.kpt_n, replace=True)
