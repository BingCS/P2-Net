...
modes = ['test', 'train']
src_scene_path = root + 'KITTI'
split_data_path = root + 'KITTI'
p2net_path = root + 'KITTI'

train_test_seq = {
    '0005': {'train': ['seq-01'], 'test': []},
    '0020': {'train': ['seq-01'], 'test': []},
    '0079': {'train': ['seq-01'], 'test': []},
    '0113': {'train': ['seq-01'], 'test': ['seq-02', 'seq-03', 'seq-04', 'seq-05']}
}

corrs_train = {}
corrs_train_filename = os.path.join(p2net_path, f'KITTI_train_0.010_corrs_7scenes.pkl')

pairs_train = {}
pairs_train_filename = os.path.join(p2net_path, f'KITTI_train_0.030_pairs_7scenes.pkl')

corrs_test = {}
corrs_test_filename = os.path.join(p2net_path, f'KITTI_test_0.010_corrs_7scenes.pkl')

pairs_test = {}
pairs_test_filename = os.path.join(p2net_path, f'KITTI_test_0.030_pairs_7scenes.pkl')
skip = 5
        ...
        for start_frame in start_frame_list:
            ...
            for frame_id in select_frame_list:
                ...
                if anc_valid_points_len >= 3000 and anc_valid_points_len <= 120000:
                    ...
                    matched_pcd_coarse, matched_pcd_fine = get_matching_indices(pos_points.astype(np.float32), anc_valid_points.astype(np.float32), 0.090, 0.006)
