...
class photaug:
    ...
    @staticmethod
    def random_contrast(image, strength_range=[0.3, 1.5]):
        delta = np.random.uniform(strength_range[0], strength_range[1])
        mean = np.mean(image)
        image = (image - mean) * delta + mean
        image = np.clip(image, 0, 255)
        return image

    @staticmethod
    def motion_blur(image, max_kernel_size=3):
...
def data_aug(xyz, num_in, num_out):
    if num_in == 115200:
        xyz_aug = np.concatenate([xyz, xyz], 0)
    else:
...
def photometric_augmentation(image, random_order=True):
    # primitives = ['random_brightness', 'random_contrast', 'motion_blur']
    primitives = ['random_brightness']
    config = {}
    config['random_brightness'] = 50
    # config['random_contrast'] = [0.3, 1.5]
    # config['motion_blur'] = 3
    ...
    for i in indices:
        aug_func = getattr(photaug, primitives[i])
        image = aug_func(image, config[primitives[i]])
