...
from networks.libs.base_lib import load_kernels
...
class KPConv(nn.Module):
    ...
    def forward(self, q_pts, s_pts, neighb_inds, x):
        ...
        # output_features = output_features / neighbor_num.unsqueeze(1).to(torch.float32)
...
def block_decider(block_name,
    ...
    elif block_name == 'HeadBlock':
        return HeadBlock(in_dim, 128, False)
...
class HeadBlock(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, use_bias):
        """
        Initialize a standard head block with its ReLU and BatchNorm.
        ...
        self.mlp = nn.Linear(self.in_dim, self.out_dim, bias=self.use_bias)
        return
    ...
    def __repr__(self):
        return 'HeadBlock(in_feat: {:d}, out_feat: {:d}), use_bias: {:s}'.format(self.in_dim, self.out_dim, str(self.use_bias))
